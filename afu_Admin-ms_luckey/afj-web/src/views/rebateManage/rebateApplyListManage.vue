    ...
    >
      ...
      <el-table-column label="申请类型" align="center">
        <template slot-scope="{ row }">
          <div>{{ row.applyType == 1 ? "积分申请" : "积分调整" }}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="利率描述" align="center">
        <template slot-scope="{ row }">
          <div>{{ row.roleDescribe }}</div>
        </template>
      </el-table-column>

      <el-table-column label="需返积分" align="center">
        <template slot-scope="{ row }">
          <div>{{ row.mortgageMoney }}</div>
        </template>
      </el-table-column>
      <el-table-column label="积分金额" align="center">
        <template slot-scope="{ row }">
          <div>{{ row.rebateMoney }}</div>
        </template>
      </el-table-column>

      <el-table-column label="流水号" align="center">
        <template slot-scope="{ row }">
          <div>{{ row.mortgageNo }}</div>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row }">
          <el-button
            ...
            type="primary"
            size="mini"
            v-if="handleBtnShow[1].isShow && row.auditStatus == 3"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      handleBtnShow: [
        {
          btnName: "RebateApplyQuery",
          isShow: false,
        },
        {
          btnName: "RebateApplyModify",
          isShow: false,
        },
      ],
      downloadLoading: false,
  ...
  methods: {
    async getList() {
      ...
      let res = await rebateApplyList({
        ...this.listQuery,
      });
      console.log(res);
      if (res.rtn == 1) {
        this.list = res.result.rebateApplyDtoList;
        this.total = res.result.total;
        this.list.forEach((item, index) => {
          if (item.auditStatus == 1) {
            item.auditStatusText = "审核中";
          } else if (item.auditStatus == 2) {
            item.auditStatusText = "审批通过";
          } else if (item.auditStatus == 3) {
            item.auditStatusText = "审核驳回";
          }
        });
        // this.list.forEach((item, index) => {
        //   if (
    ...
    toDtl(row) {
      ...
      this.$router.push({
        name: "RebateApplyDtlManage",
        query: {
          workflowId: row.workflowId,
          beforeRouter: "RebateApplyListManage",
        },
      });
    },
    ...
    handleApplyRebate(row) {
      ...
      const dataPicList = row.dataPicList;
      const dataPdfList = row.dataPdfList;
      ...
      if (dataPdfList && dataPdfList.length > 0) {
        this.dialogForm1.dataPdfList.forEach((item, index) => {
          var pdfJson = {};
          item.pdfFileList = [];
    ...
    async applyRebate() {
      this.$refs.dialogForm1.validate(async (valid) => {
        if (valid) {
          ...
          const res = await editApplyRebate(paramsData);
          if (res.rtn == 1) {
    ...
    async handleEdit(row) {
      ...
      let {
        ...
        rebateMoney,
        applyId,
      } = res.result.auditObject.rebateApplyInfo;
      let carDealersId = res.result.auditObject.carDealersInfo.carDealersId;
      var row = {
        mortgageNo,
        mortgageMoney,
        dataPicList,
      ...
      if (row.applyType == 1) {
        this.handleApplyRebate(row);
      } else {
        this.handleEditRebate(row);
      }
    },
  },
};
